...
export const reportingService = {
  ...
  // ── Fetch from backend then build CSVs ────────────────────
  async exportToCSV(tests, results, projectId) {
    // If we have DB data, fetch from backend for the authoritative records
    ...
    // Test Cases CSV
    const tcRows = [
      ['Test ID', 'Title', 'Description', 'Priority', 'Type', 'Status', 'Created By', 'Created At'],
    ];

    const tcSource = backendData?.testCases || Object.entries(tests).flatMap(([type, list]) =>
    ...
    tcSource.forEach(tc => tcRows.push([
      tc.id || tc.test_case_key || 'N/A',
      tc.name || tc.summary,
      tc.description || '',
      tc.priority || 'Medium',
      tc.type || tc.test_case_type || '',
    ...
    if (resultSource.length) {
      const resRows = [
        ['Test ID', 'Title', 'Status', 'Duration (ms)', 'Environment', 'Browser', 'Timestamp', 'Error'],
      ];
      resultSource.forEach((r, i) => resRows.push([
        r.id || r.testKey || `TC-${i + 1}`,
        r.name || r.test_name,
        (r.status || r.execution_status || '').toUpperCase(),
        r.duration || r.duration_ms || 0,
        r.env || r.environment || 'production',
        r.browser || 'chrome',
    ...
    if (bugSource.length) {
      ...
      bugSource.forEach(b => bugRows.push([b.key || b.jiraKey, b.summary, b.severity, b.priority, b.status, b.date ? new Date(b.date).toLocaleString() : '']));
      downloadCSV(makeCSV(bugRows), 'Bug_Reports.csv');
    }
  },

  // ── Fetch execution summary from backend ──────────────────
  ...
  // ── Screenshot download (client-only) ────────────────────
  ...
  // ── CSV template download (client-only) ──────────────────
